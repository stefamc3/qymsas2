...
namespace QYMSAS
{
    ...
    {
        int iduss;
        public Remisiones(int idusuario)
        {
            InitializeComponent();
            iduss = idusuario;
        }
        ...
        {
            int modifica = basededatos.ModificaRemisiones(txtdestinor.Text, dg_consulta.Rows[dg_consulta.CurrentRow.Index].Cells[0].Value.ToString());
            if (modifica > 0)
            {
